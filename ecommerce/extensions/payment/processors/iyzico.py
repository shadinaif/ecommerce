...
class Iyzico(BasePaymentProcessor):
    ...
    def retrieve_payment_info(self, token):
        ...
        logger.info('---------------type:json.loads(bytes_data) = {}'.format(type(data)))
    ...
    def get_transaction_parameters(self, basket, request=None, use_client_side_checkout=False, **kwargs):
        logger.info('---------------{}.{}'.format(type(self).__name__, inspect.stack()[0][3]))
        user = request.user
        logger.info('---------------get_transaction_parameters.request = {}'.format(request))
        logger.info('---------------get_transaction_parameters.request.GET = {}'.format(request.GET))
        logger.info('---------------get_transaction_parameters.request.POST = {}'.format(request.POST))
        logger.info('---------------get_transaction_parameters.request.user = {}'.format(request.user))
        logger.info('---------------basket = {}'.format(basket))
        logger.info('---------------basket.id = {}'.format(basket.id))
        logger.info('---------------basket.owner = {}'.format(basket.owner))
        logger.info('---------------basket.owner-type = {}'.format(type(basket.owner)))
        logger.info('---------------user.username = {}'.format(user.username))
        logger.info('---------------basket-type = {}'.format(type(basket)))
    ...
    def handle_processor_response(self, response, basket=None):
        ...
        logger.info('---------------handle_processor_response01')
        logger.info('---------------handle_processor_response02')
        ...
        #         )
        #         raise GatewayError
        logger.info('---------------handle_processor_response03')
        ...
        logger.info('---------------handle_processor_response04')
...
# class xxxIyzico(BasePaymentProcessor):
#     """
#     Iyzico REST API (May 2015)
#
#     For reference, see https://developer.iyzico.com/docs/api/.
...
#         logger.info('---------------{}.{}'.format(type(self).__name__, inspect.stack()[0][3]))
#         parameters = {
#             'payment_page_url': '/payment/iyzico/payment',
#         }
#         return parameters
#
#     def xxxxget_transaction_parameters(self, basket, request=None, use_client_side_checkout=False, **kwargs):
#         """
